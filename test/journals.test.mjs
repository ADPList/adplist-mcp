...
test("M5 registers list_journals, search_journal_learnings, and read_journal with learning intent routing", () => {
	...
	// Provenance is stated as fact so the model can reason about it itself.
	assert.match(indexSource, /generated by ADPList's AI note taker/i);
	assert.match(indexSource, /not free-form journals written by the user/i);
	assert.match(
		indexSource,
		/questions about learnings, lessons, takeaways, patterns, or insights/i,
	);
	assert.match(indexSource, /as distinct from locating one specific session in raw history/i);
});
