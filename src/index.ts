...
export class MyMCP extends McpAgent<Env, unknown, McpUserProps> {
	...
	async init() {
		...
		this.server.registerTool(
			...
			{
				description:
					"Find ADPList mentors for a user's career intent using the existing Explore personalization ranker. This can take a few seconds because it calls the live search-service. Use it when the user describes a mentorship, career transition, role, discipline, country, or language need. In intent, describe who the user is from the conversation — current role, seniority, company or industry, and what they want to achieve and why — not just topic keywords (e.g. 'senior PM at a fintech startup transitioning into UX research, wants help running first discovery interviews' rather than 'user research for startups'). Call this tool at most ONCE per user request: every call renders another full card grid in chat, and the server already retries an over-strict discipline filter automatically, so do not run multiple filter variations yourself. If results are sparse, present what came back, note the limitation, and either deep-dive the best candidates with get_mentor_profile (renders no widget) or ask the user how to adjust before searching again. Returns compact ranked mentor cards plus Algolia queryID for later booking attribution. After the cards render, consider deep-diving the top 2-3 candidates with get_mentor_profile and giving the user a short personal recommendation in chat that references their situation — do not just restate the cards. MCP Apps hosts should render the attached clean visual mentor cards; text/JSON fallback remains complete for unsupported hosts. When the widget renders, keep your chat text to about one line and never echo the raw JSON result in prose.",
